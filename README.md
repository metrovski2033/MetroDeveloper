...
### 武器エンティティ名のリスト表示
- `wpn_give list`を実行すると、現在いるマップの武器エンティティの一覧が`wpn_give_list.log`として出力される
  
  - logファイルはゲームフォルダ(MetroExodus.exe があるフォルダ)に出力される
  - 武器エンティティの一覧はマップ毎に固定のため注意
...
### 投擲アイテムのリスト表示
- `wpn_give throw_list`を実行すると、現在いるマップの投擲アイテム(ダイナマイトなど)の一覧が`wpn_give_throw_list.log`として出力される
  - logファイルはゲームフォルダ(MetroExodus.exe があるフォルダ)に出力される
