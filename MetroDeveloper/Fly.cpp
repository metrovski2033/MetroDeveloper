...
Fly::Fly()
{
	if (Utils::GetBool("other", "fly", false) || Utils::GetBool("other", "restore_deleted_commands", false))
	{
		...
		if (Utils::isRedux()) {
			...
			if (cflycam_cflycam == NULL) {
				// 48 89 5C 24 10 55 56 57 41 54 41 55 41 56 41 57 48 81 EC 40 01 00 00 - Exodus EE
				cflycam_cflycam = (_cflycam_cflycam)FindPatternInEXE(
					"\x48\x89\x5C\x24\x10\x55\x56\x57\x41\x54\x41\x55\x41\x56\x41\x57\x48\x81\xEC\x40\x01\x00\x00",
			...
			if (Utils::isExodusPatched) {
				...
				if (camera_manager_play_track == NULL) {
					// 48 89 5C 24 08 48 89 54 24 10 57 48 83 EC 50 48 8B 02 48 8B DA 48 8B 94 24 ? ? ? ? 48 8B F9 0F 29 74 24 ? 48 8B CB 0F 29 7C 24 ? 0F 28 F3 0F 28 FA FF 90 98 00 00 00 - Exodus EE
					camera_manager_play_track = (void*)FindPatternInEXE(
						"\x48\x89\x5C\x24\x08\x48\x89\x54\x24\x10\x57\x48\x83\xEC\x50\x48\x8B\x02\x48\x8B\xDA\x48\x8B\x94\x24\x00\x00\x00\x00\x48\x8B\xF9\x0F\x29\x74\x24\x00\x48\x8B\xCB\x0F\x29\x7C\x24\x00\x0F\x28\xF3\x0F\x28\xFA\xFF\x90\x98\x00\x00\x00",
						"xxxxxxxxxxxxxxxxxxxxxxxxx????xxxxxxx?xxxxxxx?xxxxxxxxxxxx");
				}
			} else {
...
void Fly::fly(const char* name, bool refly, float refly_speed, unsigned int refly_cycles)
{
	...
	if(!refly)
		...
		_this = tlsf_memalign(tlsf, 0x10, 0x100); // TODO: ����� ����� ���������� ������ ��� ������
	LeaveCriticalSection(mem);

	// � cflycam_cflycam ��������� ��� ��������� slide � scale ����� ������ � ������
...
void __fastcall Fly::exodus_cflycam_r_on_key_press(void* _this, int action, int key, int state, int resending)
{
	...
	default:
		if (cflycam_r_on_key_press_Orig) {
			typedef void(__fastcall* _cflycam_r_on_key_press)(void*, int, int, int, int);
			((_cflycam_r_on_key_press)cflycam_r_on_key_press_Orig)(_this, action, key, state, resending);
		}
		return;
	}
}
